...
bot.on("ready", () => {
  ...
  console.log(
    `Bot status: ONLINE (Serving '${guild.name}' | ${guild.id} | Bot name: ${bot.user.tag})`
  );
  ...
  setInterval(() => {
    ...
    const gifEmbed = new MessageEmbed()
      ...
      .setImage(
        loreBook.raccoonGifs[
          Math.floor(Math.random() * loreBook.raccoonGifs.length)
        ]
      )
      .setTimestamp();
  ...
  setInterval(() => {
    ...
    guild.members.fetch().then((members) => {
      ...
      let randomUser = members
        .filter(
          (member) => !member.user.bot && member._roles.includes(requiredRole)
        )
        .randomKey();
      guild.channels.fetch(spChannelId).then((channel) => {
        channel.send(
          `<@${randomUser}> Did you know ${
            loreBook.funFacts[
              Math.floor(Math.random() * loreBook.funFacts.length)
            ]
          }?`
        );
...
bot.on("messageCreate", (message) => {
  if (message.author.bot) {
    ...
    const commands = new MessageEmbed()
      ...
      .setDescription(
        "I'm still learning 👉👈 ... but this is what I can do right now: "
      )
      .addFields(
    ...
    message.channel.send(
      loreBook.jokes[Math.floor(Math.random() * loreBook.jokes.length)]
    );
